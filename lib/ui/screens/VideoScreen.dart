...
class VideoScreenState extends State<VideoScreen> {
      ...
      builder: (context, state) {
        ...
        if (state is VideoFetchFailure) {
          return const Center(
            ...
            text: 'videoNotAvailLbl',
            textAlign: TextAlign.center,
          ));
        }
        return Padding(padding: const EdgeInsets.only(bottom: 10.0, left: 30.0, right: 30.0), child: videoShimmer(context));
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: setCustomAppBar(height: 45, isBackBtn: false, label: 'videosLbl', context: context, isConvertText: true),
      body: _buildVideos(),
    );
  }
}

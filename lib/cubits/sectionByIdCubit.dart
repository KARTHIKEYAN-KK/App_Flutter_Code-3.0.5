...
class SectionByIdCubit extends Cubit<SectionByIdState> {
  ...
  void getSectionById({required String userId, required String langId, required String sectionId}) async {
    try {
      ...
      final result = await _sectionByIdRepository.getSectionById(limit: limitOfAPIData.toString(), offset: "0", userId: userId, langId: langId, sectionId: sectionId);

      emit(SectionByIdFetchSuccess(
          newsModel: result[0].newsType == "news"
              ? result[0].news!
    ...
    if (state is SectionByIdFetchSuccess) {
      try {
        final result = await _sectionByIdRepository.getSectionById(
          ...
          offset: (state as SectionByIdFetchSuccess).newsModel.isNotEmpty
              ? (state as SectionByIdFetchSuccess).newsModel.length.toString()
              : (state as SectionByIdFetchSuccess).breakNewsModel.length.toString(),
          langId: langId,
          userId: userId,
          sectionId: sectionId,
        );
        ...
        if ((state as SectionByIdFetchSuccess).newsModel.isNotEmpty) {
          ...
          updatedResults.addAll((result[0].newsType == "news" ? result[0].news : result[0].videos) as List<NewsModel>);
          emit(SectionByIdFetchSuccess(
              newsModel: updatedResults,
              totalCount: result[0].newsType == "news" ? result[0].newsTotal! : result[0].videosTotal!,
              hasMoreFetchError: false,
              hasMore: updatedResults.length < (result[0].newsType == "news" ? result[0].newsTotal! : result[0].videosTotal!),
              type: result[0].newsType!,
              breakNewsModel: []));
        } else {

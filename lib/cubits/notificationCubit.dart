...
class NotificationCubit extends Cubit<NotificationState> {
  ...
  void getNotification({required BuildContext context, required String langId}) async {
    try {
      emit(NotificationFetchInProgress());
      final result = await _notificationRepository.getNotification(limit: limitOfAPIData.toString(), offset: "0", context: context, langId: langId);
  ...
  void getMoreNotification({required BuildContext context, required String langId}) async {
    if (state is NotificationFetchSuccess) {
      try {
        final result = await _notificationRepository.getNotification(
            context: context, limit: limitOfAPIData.toString(), offset: (state as NotificationFetchSuccess).notification.length.toString(), langId: langId);

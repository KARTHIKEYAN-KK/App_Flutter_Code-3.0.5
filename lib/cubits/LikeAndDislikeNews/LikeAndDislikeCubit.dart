...
class LikeAndDisLikeCubit extends Cubit<LikeAndDisLikeState> {
  ...
  void addLikeAndDisLikeNews(NewsModel model) {
    if (state is LikeAndDisLikeFetchSuccess) {
      model.totalLikes = (int.parse(model.totalLikes.toString()) + 1).toString();
      List<NewsModel> likeDislikeTempList = [];
      likeDislikeTempList.insert(0, model);
      likeDislikeTempList.addAll((state as LikeAndDisLikeFetchSuccess).likeAndDisLike);

      emit(LikeAndDisLikeFetchSuccess(
        likeAndDisLike: List.from(likeDislikeTempList),
        hasMoreFetchError: true,
        totalLikeAndDisLikeCount: (state as LikeAndDisLikeFetchSuccess).totalLikeAndDisLikeCount,
        hasMore: (state as LikeAndDisLikeFetchSuccess).hasMore,
      ));
  ...
  //Can pass only news id here
  void removeLikeAndDisLikeNews(NewsModel model) {
    if (state is LikeAndDisLikeFetchSuccess) {
      final likeAndDisLike = (state as LikeAndDisLikeFetchSuccess).likeAndDisLike;
      likeAndDisLike.removeWhere(((element) => element.id == model.id));
      if (model.totalLikes!.isNotEmpty) model.totalLikes = (int.parse(model.totalLikes.toString()) - 1).toString();
      emit(LikeAndDisLikeFetchSuccess(
        likeAndDisLike: List.from(likeAndDisLike),
        hasMoreFetchError: true,
        totalLikeAndDisLikeCount: (state as LikeAndDisLikeFetchSuccess).totalLikeAndDisLikeCount,
        hasMore: (state as LikeAndDisLikeFetchSuccess).hasMore,
      ));

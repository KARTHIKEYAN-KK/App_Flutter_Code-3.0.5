...
class NewsByIdCubit extends Cubit<NewsByIdState> {
  ...
  Future<List<NewsModel>> getNewsById({required BuildContext context, required String newsId, required String langId, required String userId}) async {
    try {
      emit(NewsByIdFetchInProgress());
      final result = await _newsByIdRepository.getNewsById(context: context, langId: langId, newsId: newsId, userId: userId);

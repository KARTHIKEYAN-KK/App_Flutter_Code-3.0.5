...
class CommentViewState extends State<CommentView> {
  ...
  Widget allCommentView(int length) {
    return Row(
      children: [
        if (length > 0)
          Padding(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                            ...
                            suffixIcon: IconButton(
                              icon: Icon(
                                Icons.send,
                                color: comBtnEnabled ? UiUtils.getColorScheme(context).primaryContainer.withOpacity(0.8) : Colors.transparent,
                                size: 20.0,
                              ),
                              onPressed: () async {
                                if (context.read<AuthCubit>().getUserId() != "0") {
                                  context.read<SetCommentCubit>().setComment(userId: context.read<AuthCubit>().getUserId(), parentId: "0", newsId: widget.newsId, message: _commentC.text);
                                } else {
                                    ...
                                    children: [
                                      ...
                                      if (context.read<AuthCubit>().getUserId() != "0")
                                        InkWell(
                                          ...
                                          onTap: () {
                                            delAndReportCom(index: index, newsId: widget.newsId, context: context, model: model, reportC: reportC, setState: setState);
                                          },
                                        )
                                    ],
                                  ),
                              ...
                              child: InkWell(
                                child: Padding(
                                  ...
                                  child: Row(
                                    children: [
                                      Text(
                                        model.replyComList!.isNotEmpty ? "${model.replyComList!.length} " : "",
        ...
        child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              return _buildCommContainer(
                ...
                index: index,
                totalCurrentComm: state.commentNews.length,
              );
            }));
  }
    ...
    return BlocBuilder<CommentNewsCubit, CommentNewsState>(builder: (context, state) {
      if (state is CommentNewsFetchSuccess) {
        return Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [allCommentView(state.commentNews.length), profileWithSendCom(), allComListView(state)],
            ));
      }
